...
def main_dev():
	...
	parser.add_argument('input_files', metavar='SSR json files', type=str, nargs='+', help='SSR gui-config.json or json-stored files')
	...
	if args.target and os.path.exists(args.target[0]):
		with open(args.target[0], 'rb') as f:
			d = json.load(f)
		...
		print('*** Pls input correct target SSR gui-config.json path and file name...')
		sys.exit(1)
	else:
		target = None

	if not args.input_files and os.path.exists('gui-config.json'):
...
def end(dest_list, dup_list, only_test):
	if len(dup_list) == 0:
...
def main():
	...
	if input_jsons and isinstance(input_jsons, list):
		...
		end(dest_list, dup_list, only_test)
	else:
		print('Something wrong, pls check check...')
		parser.print_help()
		sys.exit(1)
